...
try:
    from news import get_groww_company_outlook, get_pulse_news
except Exception as exc:
...
if ENABLE_SOCKETIO:
    try:
        ...
        socketio = None
        logger.warning(
            "flask_socketio present but failed to initialize; socket.io endpoints will be degraded."
        )
else:
...
def analyze_ticker(ticker: str) -> dict:
    ...
    current_price = info.get("currentPrice") or (
        df["Close"].iloc[-1] if not df.empty else 0.0
    )
    ...
    intrinsic_price, diff_percent = valuation_engine.calculate_intrinsic_value(
        info, current_price
    )
...
def api_groww_news_outlook():
    ...
    try:
        ...
        outlook = get_groww_company_outlook(
            company=company, company_url=company_url, page_excerpt=page_excerpt
        )
